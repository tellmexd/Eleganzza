...
// ── DraftItemRow — top-level so React never remounts it on parent re-render ──
// Receives draft + setDraft as props instead of closing over them.
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 80px 70px 70px 24px",
      ...
      <Field label={idx===0?"PERFUME / MODELO":""} type="text" value={it.name}
        onChange={e=>{
          ...
          setDraft(p=>({...p,items:p.items.map(x=>x.id===it.id?{...x,name:v}:x)}));
        }}/>
      <Field label={idx===0?"USD UNIT":""} prefix="$" value={it.priceUSD}
        onChange={e=>{
          const v=parseFloat(e.target.value)||0;
          setDraft(p=>({...p,items:p.items.map(x=>x.id===it.id?{...x,priceUSD:v}:x)}));
        }}/>
      <Field label={idx===0?"UNIDADES":""} step="1" value={it.units}
        onChange={e=>{
          const v=parseFloat(e.target.value)||0;
          setDraft(p=>({...p,items:p.items.map(x=>x.id===it.id?{...x,units:v}:x)}));
        }}/>
      <Field label={idx===0?"PESO (Kg)":""} step="0.01" value={it.weightKg??0.82}
        onChange={e=>{
          const v=parseFloat(e.target.value)||0;
          setDraft(p=>({...p,items:p.items.map(x=>x.id===it.id?{...x,weightKg:v}:x)}));
        }}/>
      <button
        onClick={()=>setDraft(p=>({...p,items:p.items.filter(x=>x.id!==it.id)}))}
        disabled={!canRemove}
...
// ── ItemRow — top-level for the same reason. Receives updItem + canRemove as props. ──
function ImportItemRow({it, bR, updItem, canRemove, onRemove}){
  const ri = bR.items.find(x=>x.id===it.id) || it;
  const [d, setD] = useState({
    name:it.name, priceUSD:it.priceUSD, units:it.units,
    weightKg:it.weightKg??0.82, discount:it.discount??0,
  });

  useEffect(()=>{
    const tag = document.activeElement && document.activeElement.tagName;
    if(tag==="INPUT") return;
    setD({name:it.name, priceUSD:it.priceUSD, units:it.units,
      weightKg:it.weightKg??0.82, discount:it.discount??0});
  // eslint-disable-next-line
  },[it.name, it.priceUSD, it.units, it.weightKg, it.discount]);
      ...
      {ri.unitCost>0 && (
        <div style={{display:"flex",gap:14,flexWrap:"wrap",paddingTop:2}}>
          <span style={{fontSize:8.5,color:C.dim}}>
            Peso: <span style={{color:C.text}}>{((ri.pesoItem||ri.units*0.82)||0).toFixed(2)} kg</span>
          </span>
          <span style={{fontSize:8.5,color:C.dim}}>
            Costo producto: <span style={{color:C.text}}>{sol(ri.prodCost,2)}</span>
          </span>
          <span style={{fontSize:8.5,color:C.dim}}>
            Logística: <span style={{color:C.text}}>{sol(ri.logShare,2)}</span>
          </span>
...
function ImportView({imp,setImp,settings}){
  ...
  // ── pure display helpers (no hooks — safe as inline consts) ──
          ...
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            <div style={{background:C.card,border:`1px solid ${C.brd}`,borderRadius:4,padding:"18px 20px"}}>
              ...
              {(selected.items||[]).map(it=>(
                <ImportItemRow key={it.id} it={it} bR={R} updItem={updItem} canRemove={(selected.items||[]).length>1} onRemove={()=>removeItem(it.id)}/>
              ))}
